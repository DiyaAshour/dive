...
try {
  ...
  if (previous?.hotelMemberships.length) {
    for (const membership of previous.hotelMemberships) await cleanupHotel(membership.hotelId);
  }
  ...
  const hotel = await server.createHotel(registered.user.id, {
    name: "Reservation Operations Smoke Hotel",
    city: "Amman",
    countryCode: "JO",
    address: "Reservation smoke test address, Amman",
    timezone: "UTC",
    currency: "JOD",
  });
  hotelId = hotel.id;
  ...
  const rateA = await server.createRatePlan(registered.user.id, hotel.id, {
    roomTypeId: roomA.id,
    name: "Flexible King",
    code: "FLEX-K",
    refundable: true,
  ...
  const rateB = await server.createRatePlan(registered.user.id, hotel.id, {
    roomTypeId: roomB.id,
    name: "Flexible Twin",
    code: "FLEX-T",
    refundable: true,
  ...
  await server.upsertCalendar(registered.user.id, hotel.id, {
    entries: dates.flatMap((date, index) => [
  ...
  const noShowHold = await server.createBookingHold({
    ...
    adults: 2,
    children: 0,
    guestName: "No Show Smoke Guest",
    guestEmail: "no-show-smoke@handmekey.invalid",
  ...
  const futureHold = await server.createBookingHold({
    hotelId: hotel.id,
    roomTypeId: roomA.id,
    ratePlanId: rateA.id,
    ...
    adults: 2,
    children: 0,
    guestName: "Modify Cancel Smoke Guest",
    guestEmail: "modify-cancel-smoke@handmekey.invalid",
    paymentMode: "PAY_AT_HOTEL",
  }, {idempotencyKey: "reservation-smoke-future-hold"});
  ...
  await assert.rejects(
    () => server.markHotelReservationNoShow(registered.user.id, hotel.id, futureBooking.id, "reservation-smoke-too-early"),
    (error: unknown) => error instanceof server.ApplicationError && error.code === "NO_SHOW_TOO_EARLY",
  );

  const modifiedArrival = utcDay(4);
  const modifiedDeparture = utcDay(6);
  const modified = await server.modifyHotelReservation(
    registered.user.id,
    hotel.id,
    futureBooking.id,
    {roomTypeId: roomB.id, ratePlanId: rateB.id, arrival: modifiedArrival, departure: modifiedDeparture, adults: 2, children: 0},
    "reservation-smoke-modify",
  );
...
function roomInput(name: string, code: string, quantity: number) {
  return {
    name,
    code,
    description: `${name} configured for the reservation-management integration smoke flow with complete occupancy and room details.`,
    ...
    active: true,
    beds: [{area: "Bedroom 1", type: code === "TWIN" ? "SINGLE" as const : "KING" as const, quantity: code === "TWIN" ? 2 : 1, sortOrder: 0}],
    amenities: [
