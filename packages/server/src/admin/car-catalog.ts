...
export async function getAdminCarCatalogCoverage(adminUserId: string) {
  ...
  const [fleet, links, catalog] = await Promise.all([
    db.carVehicle.findMany({select: {id: true, make: true, model: true, year: true, category: true, status: true}, orderBy: [{make: "asc"}, {model: "asc"}, {year: "desc"}], take: 1000}),
    db.carVehicleCatalogLink.findMany({select: {vehicleId: true, catalogVehicleId: true}}),
    db.carCatalogVehicle.findMany({where: {active: true}, select: {id: true, make: true, model: true, year: true, trim: true, primaryImageUrl: true, exterior360Available: true, interior360Available: true, assets: {where: {active: true}, select: {type: true}}}}),
  ]);
  const linkedIds = new Set(links.map((link) => link.vehicleId));
  const catalogWithHero = catalog.filter((vehicle) => vehicle.primaryImageUrl || vehicle.assets.some((asset) => asset.type === "HERO" || asset.type === "EXTERIOR_FRONT_LEFT" || asset.type === "EXTERIOR_FRONT")).length;
  const unlinkedFleet = fleet.filter((vehicle) => !linkedIds.has(vehicle.id)).slice(0, 100);
  return {
    fleetVehicles: fleet.length,
    linkedFleetVehicles: fleet.length - unlinkedFleet.length,
    unlinkedFleetVehicles: unlinkedFleet.length,
    catalogVehicles: catalog.length,
    heroReadyVehicles: catalogWithHero,
