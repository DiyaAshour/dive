...
export async function verifyExactImaginVehicle(selectionInput: ImaginVehicleSelection) {
  const selection = normalizeSelection(selectionInput);
  const probeUrl = imageUrl(selection, {angle: "23", width: 150, zoomType: "relative"});
  const response = await fetch(probeUrl, {method: "HEAD", cache: "no-store"});
  const requestFound = headerBoolean(response.headers.get("x-imaginstudio-request-found"));
  const requestResolved = headerBoolean(response.headers.get("x-imaginstudio-request-resolved"));
  const performedMatches = normalizePerformedMatches(response.headers.get("x-imaginstudio-performed-matches"));

  // IMAGIN may deliberately return a near-match or speedShape while a model is
  // being produced. HandMeKey only publishes the visual when the CDN confirms
  // that the original exact request was entirely satisfied.
  return {
    exact: response.ok && requestFound === true && requestResolved !== false && performedMatches === null,
...
function headerBoolean(value: string | null) {
  if (value?.toLowerCase() === "true") return true;
  if (value?.toLowerCase() === "false") return false;
  return null;
}

function normalizePerformedMatches(value: string | null) {
