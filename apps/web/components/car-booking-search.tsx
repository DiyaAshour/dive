...
import {CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, Clock, MapPin, Search, SlidersHorizontal, Users, X} from "lucide-react";
import styles from "./car-booking-search.module.css";
import extrasStyles from "./car-booking-search-extras.module.css";

type Locale = "ar" | "en";
type Picker = "pickupDate" | "pickupTime" | "returnDate" | "returnTime" | "age" | "brand" | "filters" | null;
...
type PolicyKey = "freeCancellation"|"unlimitedMileage"|"airportPickup"|"payAtPickup";
...
const EMPTY_EXTRAS:Record<ExtraKey,boolean>={"child-seat":false,"infant-seat":false,"booster-seat":false,gps:false,"additional-driver":false};
const EMPTY_POLICIES:Record<PolicyKey,boolean>={freeCancellation:false,unlimitedMileage:false,airportPickup:false,payAtPickup:false};
...
export function CarBookingSearch({locale,defaultPickupDate,defaultReturnDate}:Props){
  ...
  const [returnDate,setReturnDate]=useState(defaultReturnDate>defaultPickupDate?defaultReturnDate:addDays(defaultPickupDate,1));
  ...
  const [extraFilters,setExtraFilters]=useState<Record<ExtraKey,boolean>>({...EMPTY_EXTRAS});
  const [policyFilters,setPolicyFilters]=useState<Record<PolicyKey,boolean>>({...EMPTY_POLICIES});
  ...
  const copy=ar?{
    ...
    dates:"التواريخ",flexible:"مرن",exactDates:"تواريخ محددة",day:"يوم",days:"أيام",filters:"فلاتر وإضافات",filtersHint:"إضافات وشروط العرض",extrasTitle:"مستلزمات إضافية",extrasNote:"الإضافات حسب توفر شركة التأجير وقد تكون برسوم إضافية.",policiesTitle:"شروط العرض",reset:"إعادة ضبط",done:"تم",
    childSeat:"كرسي أطفال",infantSeat:"مقعد رضيع",boosterSeat:"مقعد معزز (Booster)",gps:"جهاز GPS",additionalDriver:"سائق إضافي",freeCancellation:"إلغاء مجاني",unlimitedMileage:"كيلومترات غير محدودة",airportPickup:"استلام من المطار",payAtPickup:"الدفع عند الاستلام",
    ...
    dates:"Dates",flexible:"Flexible",exactDates:"Exact dates",day:"day",days:"days",filters:"Filters & extras",filtersHint:"Extras and offer terms",extrasTitle:"Optional extras",extrasNote:"Extras depend on rental-company availability and may have an additional charge.",policiesTitle:"Offer terms",reset:"Reset",done:"Done",
    childSeat:"Child seat",infantSeat:"Infant seat",boosterSeat:"Booster seat",gps:"GPS",additionalDriver:"Additional driver",freeCancellation:"Free cancellation",unlimitedMileage:"Unlimited mileage",airportPickup:"Airport pickup",payAtPickup:"Pay at pickup",
  ...
  const policyOptions:Array<{key:PolicyKey;label:string}>=[
    {key:"freeCancellation",label:copy.freeCancellation},{key:"unlimitedMileage",label:copy.unlimitedMileage},{key:"airportPickup",label:copy.airportPickup},{key:"payAtPickup",label:copy.payAtPickup},
  ];
  ...
  const selectedFilterCount=Object.values(extraFilters).filter(Boolean).length+Object.values(policyFilters).filter(Boolean).length;
  ...
  function chooseDate(value:string){if(picker==="pickupDate"){setPickupDate(value);if(returnDate<=value)setReturnDate(addDays(value,1));setPicker("returnDate");return;}if(picker==="returnDate"){if(value<=pickupDate)return;setReturnDate(value);setPicker(null);}}
  ...
  function toggleExtra(key:ExtraKey){setExtraFilters((current)=>({...current,[key]:!current[key]}));}
  function togglePolicy(key:PolicyKey){setPolicyFilters((current)=>({...current,[key]:!current[key]}));}
  function resetFilters(){setExtraFilters({...EMPTY_EXTRAS});setPolicyFilters({...EMPTY_POLICIES});}
  ...
  return <div className={styles.root} id="car-search">
    <form className={styles.dock} action="/cars" method="get">
      ...
      {extraOptions.filter(({key})=>extraFilters[key]).map(({key})=><input key={key} type="hidden" name="extras" value={key}/>)}
      {policyFilters.freeCancellation&&<input type="hidden" name="freeCancellation" value="1"/>}{policyFilters.unlimitedMileage&&<input type="hidden" name="unlimitedMileage" value="1"/>}{policyFilters.airportPickup&&<input type="hidden" name="airportPickup" value="1"/>}{policyFilters.payAtPickup&&<input type="hidden" name="payAtPickup" value="1"/>}
      <div className={extrasStyles.actions}>
        <button type="button" className={extrasStyles.filterTrigger} onClick={()=>setPicker("filters")} aria-label={copy.filters}><SlidersHorizontal size={19}/>{selectedFilterCount>0&&<b>{selectedFilterCount}</b>}<span>{copy.filters}</span></button>
        <button className={`${styles.searchButton} ${extrasStyles.searchAction}`} type="submit"><Search size={21}/><span>{copy.search}</span></button>
      </div>
    </form>

    {picker&&<div className={`${styles.sheetBackdrop} ${extrasStyles.fullOverlay}`} role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget)setPicker(null);}}><section className={`${styles.sheet} ${datePickerOpen?styles.dateSheet:""} ${picker==="filters"?extrasStyles.filterSheet:""}`} role="dialog" aria-modal="true" aria-label={pickerTitle(picker,copy)}><div className={styles.sheetHandle}/>
      {datePickerOpen?<><div className={styles.segmented}><button type="button" className={dateMode==="dates"?styles.segmentActive:""} onClick={()=>{setDateMode("dates");setFlexDays(0);}}>{copy.dates}</button><button type="button" className={dateMode==="flexible"?styles.segmentActive:""} onClick={()=>{setDateMode("flexible");if(flexDays===0)setFlexDays(1);}}>{copy.flexible}</button></div><div className={styles.dateSheetLabel}>{picker==="pickupDate"?copy.choosePickupDate:copy.chooseReturnDate}</div><CalendarPicker month={calendarMonth} setMonth={setCalendarMonth} months={copy.months} weekdays={copy.weekdays} pickupDate={pickupDate} returnDate={returnDate} activeDate={picker} onSelect={chooseDate}/><div className={styles.flexRow}>{FLEX_OPTIONS.map((days)=><button key={days} type="button" className={flexDays===days?styles.flexActive:""} onClick={()=>selectFlex(days)}>{days===0?<><CalendarDays size={14}/>{copy.exactDates}</>:`± ${days} ${days===1?copy.day:copy.days}`}</button>)}</div></>:<><div className={styles.sheetHead}><div><span className={styles.sheetEyebrow}>HandMeKey Cars</span><h2>{pickerTitle(picker,copy)}</h2></div><button type="button" onClick={()=>setPicker(null)} aria-label={copy.close}><X size={19}/></button></div>
        ...
        {picker==="filters"&&<div className={extrasStyles.filterBody}><section><div className={extrasStyles.sectionHead}><strong>{copy.extrasTitle}</strong><span>{copy.extrasNote}</span></div><div className={extrasStyles.optionGrid}>{extraOptions.map(({key,label})=><button type="button" key={key} className={extraFilters[key]?extrasStyles.optionActive:""} onClick={()=>toggleExtra(key)}><span>{label}</span><span className={extrasStyles.checkBox}>{extraFilters[key]&&<Check size={15}/>}</span></button>)}</div></section><section><div className={extrasStyles.sectionHead}><strong>{copy.policiesTitle}</strong></div><div className={extrasStyles.optionGrid}>{policyOptions.map(({key,label})=><button type="button" key={key} className={policyFilters[key]?extrasStyles.optionActive:""} onClick={()=>togglePolicy(key)}><span>{label}</span><span className={extrasStyles.checkBox}>{policyFilters[key]&&<Check size={15}/>}</span></button>)}</div></section><div className={extrasStyles.filterFooter}><button type="button" onClick={resetFilters}>{copy.reset}</button><button type="button" onClick={()=>setPicker(null)}>{copy.done}{selectedFilterCount>0&&<span>{selectedFilterCount}</span>}</button></div></div>}
      </>}
    </section></div>}
  </div>;
}
...
function CalendarPicker({month,setMonth,months,weekdays,pickupDate,returnDate,activeDate,onSelect}:{month:Date;setMonth:(value:Date)=>void;months:string[];weekdays:string[];pickupDate:string;returnDate:string;activeDate:"pickupDate"|"returnDate";onSelect:(value:string)=>void}){const year=month.getUTCFullYear(),monthIndex=month.getUTCMonth();const daysInMonth=new Date(Date.UTC(year,monthIndex+1,0)).getUTCDate();const firstDay=new Date(Date.UTC(year,monthIndex,1)).getUTCDay();const cells:Array<number|null>=[...Array.from({length:firstDay},()=>null),...Array.from({length:daysInMonth},(_,i)=>i+1)];const today=currentDateValue();const currentMonth=monthStart(today);const canGoPrevious=month.getTime()>currentMonth.getTime();return <div className={styles.calendar}><div className={styles.calendarHead}><button type="button" disabled={!canGoPrevious} onClick={()=>setMonth(new Date(Date.UTC(year,monthIndex-1,1)))} aria-label="Previous"><ChevronLeft size={19}/></button><strong>{months[monthIndex]??""} {year}</strong><button type="button" onClick={()=>setMonth(new Date(Date.UTC(year,monthIndex+1,1)))} aria-label="Next"><ChevronRight size={19}/></button></div><div className={styles.weekdays}>{weekdays.map((day,index)=><span key={`${day}-${index}`}>{day}</span>)}</div><div className={styles.calendarGrid}>{cells.map((day,index)=>{if(day===null)return <span key={`empty-${index}`}/>;const value=`${year}-${String(monthIndex+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;const disabled=value<today||(activeDate==="returnDate"&&value<=pickupDate);const active=value===pickupDate||value===returnDate;const inRange=value>pickupDate&&value<returnDate;const className=[active?styles.dateActive:"",inRange?styles.dateRange:""].filter(Boolean).join(" ");return <button type="button" key={value} disabled={disabled} className={className} onClick={()=>onSelect(value)}><span>{day}</span></button>;})}</div></div>;}
