...
export function CarBookingSearch({locale, defaultPickupDate, defaultReturnDate}: Props) {
  ...
  const copy = ar ? {
    ...
    chooseAge: "اختر عمر السائق", done: "تم", localTime: "جميع الأوقات بالتوقيت المحلي", close: "إغلاق",
    ageNote: "اختر الفئة العمرية للسائق الأساسي", brandTitle: "اختر الماركة", months:["يناير","فبراير","مارس","أبريل","مايو","يونيو","يوليو","أغسطس","سبتمبر","أكتوبر","نوفمبر","ديسمبر"],
    ...
    chooseAge: "Choose driver age", done: "Done", localTime: "All times are local", close: "Close",
    ageNote: "Choose the age range of the main driver", brandTitle: "Choose brand", months:["January","February","March","April","May","June","July","August","September","October","November","December"],
    weekdays:["S","M","T","W","T","F","S"],
  };

  const filteredBrands = useMemo(() => {
    const query = brandQuery.trim().toLowerCase();
    return query ? CAR_BRANDS.filter((item) => item.toLowerCase().includes(query)) : CAR_BRANDS;
  }, [brandQuery]);
  ...
  function openDate(kind:"pickupDate"|"returnDate") {
    const value=kind==="pickupDate"?pickupDate:returnDate;
  ...
  function chooseDate(value:string) {
    if (picker === "pickupDate") {
      ...
      setPicker("pickupTime");
    } else if (picker === "returnDate") {
      if (value <= pickupDate) return;
      setReturnDate(value);
      setPicker("returnTime");
    }
  }

  function chooseTime(value:string) {
    if (picker === "pickupTime") setPickupTime(value);
    if (picker === "returnTime") setReturnTime(value);
    setPicker(null);
  }
  ...
  return <div className={styles.root}>
    <form className={styles.dock} action="/cars" method="get">
      ...
      <PickerField icon={<CalendarDays size={15}/>} label={copy.pickupDate} value={formatDate(pickupDate,locale)} onClick={()=>openDate("pickupDate")}/>
      <PickerField icon={<Clock size={15}/>} label={copy.pickupTime} value={pickupTime} onClick={()=>setPicker("pickupTime")}/>
      <PickerField icon={<CalendarDays size={15}/>} label={copy.returnDate} value={formatDate(returnDate,locale)} onClick={()=>openDate("returnDate")}/>
      ...
      <div className={`${styles.field} ${styles.ageField}`}>
        ...
        <button type="button" className={styles.fieldButton} onClick={()=>setPicker("age")}><strong>{ageLabel(driverAge,ar)}</strong><ChevronDown size={15}/></button>
      </div>
      ...
      <div className={`${styles.field} ${styles.brandField}`}>
        ...
        <button className={`${styles.brandTrigger} ${brand ? styles.brandTriggerSelected : ""}`} type="button" onClick={()=>setPicker("brand")}>
      ...
      <input type="hidden" name="pickupDate" value={pickupDate}/><input type="hidden" name="pickupTime" value={pickupTime}/>
      <input type="hidden" name="returnDate" value={returnDate}/><input type="hidden" name="returnTime" value={returnTime}/>
      <input type="hidden" name="driverAge" value={driverAge}/>{brand && <input type="hidden" name="brand" value={brand}/>} 
      <button className={styles.searchButton} type="submit"><Search size={20}/><span>{copy.search}</span></button>
    </form>

    {picker && <div className={styles.sheetBackdrop} role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget)setPicker(null);}}>
      <section className={styles.sheet} role="dialog" aria-modal="true" aria-label={pickerTitle(picker,copy)}>
        <div className={styles.sheetHandle}/>
        <div className={styles.sheetHead}><div><span className={styles.sheetEyebrow}>HandMeKey Cars</span><h2>{pickerTitle(picker,copy)}</h2></div><button type="button" onClick={()=>setPicker(null)} aria-label={copy.close}><X size={19}/></button></div>

        {(picker==="pickupDate"||picker==="returnDate") && <CalendarPicker month={calendarMonth} setMonth={setCalendarMonth} locale={locale} months={copy.months} weekdays={copy.weekdays} selected={picker==="pickupDate"?pickupDate:returnDate} minDate={picker==="returnDate"?addDays(pickupDate,1):undefined} onSelect={chooseDate}/>} 
        ...
        {picker==="brand" && <div className={styles.brandSheet}>
          <label className={styles.brandSearch}><Search size={17}/><input autoFocus value={brandQuery} onChange={(event)=>setBrandQuery(event.target.value)} placeholder={copy.brandSearch}/>{brandQuery&&<button type="button" onClick={()=>setBrandQuery("")}><X size={14}/></button>}</label>
          <button type="button" className={`${styles.brandSheetOption} ${!brand?styles.brandSheetOptionActive:""}`} onClick={()=>{setBrand("");setBrandQuery("");setPicker(null);}}><span>{copy.anyBrand}</span>{!brand&&<Check size={16}/>}</button>
          <div className={styles.brandGrid}>{filteredBrands.map((item)=>{const active=brand===item;return <button type="button" key={item} className={`${styles.brandSheetOption} ${active?styles.brandSheetOptionActive:""}`} onClick={()=>{setBrand(item);setBrandQuery("");setPicker(null);}}><BrandBadge brand={item}/><span>{item}</span>{active&&<Check size={16}/>}</button>;})}</div>
...
function CalendarPicker({month,setMonth,locale,months,weekdays,selected,minDate,onSelect}:{month:Date;setMonth:(value:Date)=>void;locale:Locale;months:string[];weekdays:string[];selected:string;minDate?:string;onSelect:(value:string)=>void}) {
  const year=month.getUTCFullYear(), monthIndex=month.getUTCMonth();
  const daysInMonth=new Date(Date.UTC(year,monthIndex+1,0)).getUTCDate();
  const firstDay=new Date(Date.UTC(year,monthIndex,1)).getUTCDay();
  const cells=[...Array(firstDay).fill(null),...Array.from({length:daysInMonth},(_,i)=>i+1)];
  return <div className={styles.calendar}>
    <div className={styles.calendarHead}><button type="button" onClick={()=>setMonth(new Date(Date.UTC(year,monthIndex-1,1)))} aria-label="Previous"><ChevronLeft size={19}/></button><strong>{months[monthIndex]} {year}</strong><button type="button" onClick={()=>setMonth(new Date(Date.UTC(year,monthIndex+1,1)))} aria-label="Next"><ChevronRight size={19}/></button></div>
    <div className={styles.weekdays}>{weekdays.map((day,index)=><span key={`${day}-${index}`}>{day}</span>)}</div>
    <div className={styles.calendarGrid}>{cells.map((day,index)=>{if(day===null)return <span key={`empty-${index}`}/>;const value=`${year}-${String(monthIndex+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;const disabled=Boolean(minDate&&value<minDate);const active=value===selected;return <button type="button" key={value} disabled={disabled} className={active?styles.dateActive:""} onClick={()=>onSelect(value)}><span>{day}</span></button>;})}</div>
  </div>;
}
...
function monthStart(value:string){const [year,month]=value.split("-").map(Number);return new Date(Date.UTC(year,month-1,1));}
function addDays(value:string,days:number){const [year,month,day]=value.split("-").map(Number);const date=new Date(Date.UTC(year,month-1,day+days));return `${date.getUTCFullYear()}-${String(date.getUTCMonth()+1).padStart(2,"0")}-${String(date.getUTCDate()).padStart(2,"0")}`;}
function formatDate(value:string,locale:Locale){const [year,month,day]=value.split("-");return locale==="ar"?`${day}/${month}/${year}`:`${day}/${month}/${year}`;}
