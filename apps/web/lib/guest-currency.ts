import {guestIntlLocale, type GuestCurrency, type GuestLocale} from "./guest-market";

// Reference display rates: units of currency per 1 USD. These values are deliberately
// presentation-only. Booking, payment, refund, wallet and payout ledgers always keep the
// hotel's authoritative ISO currency and amount. Production can replace these reference
// values with a certified FX feed without changing booking math.
const USD_REFERENCE_RATES: Readonly<Record<string, number>> = {
  ...
  CNY:7.18,HKD:7.82,TWD:31.8,JPY:147.5,KRW:1385,SGD:1.29,MYR:4.22,THB:32.4,IDR:16250,PHP:57.2,VND:26300,INR:88.0,PKR:280,BDT:122.2,LKR:300,NPR:140.8,
  EUR:0.86,GBP:0.74,CHF:0.80,SEK:9.35,NOK:9.9,DKK:6.42,PLN:3.68,CZK:20.9,HUF:333,RON:4.37,BGN:1.68,RSD:100,ALL:83.5,BAM:1.68,MKD:52.8,ISK:122,
  TRY:41.2,RUB:81.5,UAH:41.3,GEL:2.70,AMD:383,AZN:1.70,KZT:538,KGS:87.4,UZS:12450,
  AUD:1.53,NZD:1.69,CAD:1.38,MXN:18.7,BRL:5.42,ARS:1350,CLP:970,COP:4100,PEN:3.52,UYU:40.1,PYG:7900,BOB:6.91,CRC:505,DOP:60.0,GTQ:7.66,HNL:26.0,NIO:36.8,PAB:1,
  ZAR:17.7,EGP:48.5,MAD:9.05,DZD:130,TND:2.90,NGN:1530,GHS:11.2,KES:129,TZS:2500,UGX:3550,RWF:1450,ETB:141,BWP:13.7,NAD:17.7,MUR:45.8,MZN:63.9,ZMW:23.5,XAF:564,XOF:564,
...
export function convertCurrency(amount: number, sourceCurrency: string, targetCurrency: string): number | null {
  ...
  return roundDisplay((amount / sourceRate) * targetRate, target);
}
...
export function hasReferenceRate(currency: string): boolean {
  return Boolean(USD_REFERENCE_RATES[currency.toUpperCase()]);
}
