  ...
  return <main className="blogExperience" dir={rtl?"rtl":"ltr"}>
    ...
    <section className="shell blogListing">
      {!featured?<div className="blogEmpty"><BookOpen size={34}/><strong>{c.empty}</strong></div>:<>
        <article className="blogFeatured">{featured.coverImageUrl?<img src={featured.coverImageUrl} alt={featured.coverImageAlt||featured.title}/>:<div className="blogImagePlaceholder"><Sparkles size={34}/></div>}<div><span className="blogCategory">{c.featured} · {featured.category}</span><h2><Link href={`/blog/${locale}/${featured.slug}`}>{featured.title}</Link></h2><p>{featured.excerpt}</p><div className="blogMeta"><CalendarDays size={15}/>{formatDate(featured.publishedAt,locale)} · {readingMinutes(featured.body)} min</div><Link className="blogReadLink" href={`/blog/${locale}/${featured.slug}`}>{c.read}<ArrowRight size={16}/></Link></div></article>
        <div className="blogGrid">{rest.map((post)=><article className="blogCard" key={post.id}>{post.coverImageUrl?<img src={post.coverImageUrl} alt={post.coverImageAlt||post.title} loading="lazy"/>:<div className="blogImagePlaceholder"><BookOpen size={28}/></div>}<div><span className="blogCategory">{post.category}</span><h2><Link href={`/blog/${locale}/${post.slug}`}>{post.title}</Link></h2><p>{post.excerpt}</p><div className="blogMeta"><CalendarDays size={14}/>{formatDate(post.publishedAt,locale)} · {readingMinutes(post.body)} min</div><Link className="blogReadLink" href={`/blog/${locale}/${post.slug}`}>{c.read}<ArrowRight size={15}/></Link></div></article>)}</div>
      </>}
    </section>
  </main>;
}
