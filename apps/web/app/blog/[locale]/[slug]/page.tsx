...
const isBlogLocale=(value:string):value is Locale=>value==="en"||value==="ar"||value==="es";

export async function generateMetadata({params}:{params:Promise<{locale:string;slug:string}>}):Promise<Metadata>{
  const {locale:raw,slug}=await params; if(!isBlogLocale(raw))return {};
  try{
    const locale=raw; const post=await getPublishedBlogPost(locale,slug); const canonical=siteUrl(`/blog/${locale}/${post.slug}`);
    const image=post.coverImageUrl?[{url:post.coverImageUrl,alt:post.coverImageAlt||post.title}]:null;
    return {title:post.seoTitle,description:post.seoDescription,alternates:{canonical},openGraph:{type:"article",url:canonical,title:post.seoTitle,description:post.seoDescription,siteName:"HandMeKey",locale:locale==="ar"?"ar_JO":locale==="es"?"es_ES":"en_US",modifiedTime:post.updatedAt.toISOString(),authors:[post.authorName],tags:post.tags,...(post.publishedAt?{publishedTime:post.publishedAt.toISOString()}:{}),...(image?{images:image}:{})},twitter:{card:post.coverImageUrl?"summary_large_image":"summary",title:post.seoTitle,description:post.seoDescription,...(post.coverImageUrl?{images:[post.coverImageUrl]}:{})}};
  }catch{return {};}
}

export default async function BlogArticlePage({params}:{params:Promise<{locale:string;slug:string}>}){
  const {locale:raw,slug}=await params; if(!isBlogLocale(raw))notFound(); const locale=raw;
  ...
  const c=locale==="ar"
    ?{back:"العودة إلى الدليل",min:"دقائق قراءة",updated:"آخر تحديث",related:"أدلة مرتبطة",read:"اقرأ الدليل",search:"ابحث عن فندق",rewards:"مكافآت HandMeKey",guide:"دليل السفر"}
    :locale==="es"
      ?{back:"Volver a la guía",min:"min de lectura",updated:"Última actualización",related:"Guías relacionadas",read:"Leer guía",search:"Buscar hoteles",rewards:"HandMeKey Rewards",guide:"Guía de viaje"}
      :{back:"Back to Travel Guide",min:"min read",updated:"Updated",related:"Related guides",read:"Read guide",search:"Search hotels",rewards:"HandMeKey Rewards",guide:"Travel Guide"};
  ...
  const breadcrumbs={"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"HandMeKey",item:siteUrl()},{"@type":"ListItem",position:2,name:c.guide,item:siteUrl(`/blog/${locale}`)},{"@type":"ListItem",position:3,name:post.title,item:url}]};
  ...
  return <main className="blogExperience" dir={rtl?"rtl":"ltr"} lang={locale}>
    ...
    <article className="blogArticle shell">
      ...
      <div className="blogArticleLayout"><BlogArticleBody body={post.body}/><aside className="blogArticleRail"><div><strong>{c.updated}</strong><span>{formatDate(post.updatedAt,locale)}</span></div><Link href="/search">{c.search}</Link><Link href={rewardsHref}>{c.rewards}</Link>{post.tags.length>0&&<div className="blogTags">{post.tags.map((tag)=><span key={tag}>{tag}</span>)}</div>}</aside></div>
