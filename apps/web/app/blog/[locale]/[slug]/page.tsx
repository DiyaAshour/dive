...
import {ArrowLeft, ArrowRight, BookOpen, CalendarDays, Clock3, UserRound} from "lucide-react";
...
export async function generateMetadata({params}:{params:Promise<{locale:string;slug:string}>}):Promise<Metadata>{
  ...
  try{
    ...
    return {title:post.seoTitle,description:post.seoDescription,alternates:{canonical},openGraph:{type:"article",url:canonical,title:post.seoTitle,description:post.seoDescription,siteName:"HandMeKey",locale:locale==="ar"?"ar_JO":"en_US",publishedTime:post.publishedAt?.toISOString(),modifiedTime:post.updatedAt.toISOString(),authors:[post.authorName],tags:post.tags,images:image},twitter:{card:post.coverImageUrl?"summary_large_image":"summary",title:post.seoTitle,description:post.seoDescription,images:post.coverImageUrl?[post.coverImageUrl]:undefined}};
  }catch{return {};}
}
...
export default async function BlogArticlePage({params}:{params:Promise<{locale:string;slug:string}>}){
  ...
  const related=await listRelatedPublishedBlogPosts(locale,post.id,post.category,3); const rtl=locale==="ar"; const minutes=readingMinutes(post.body);
  ...
  return <main className="blogExperience" dir={rtl?"rtl":"ltr"}>
    ...
    <article className="blogArticle shell">
      ...
      <header className="blogArticleHead"><span className="blogCategory">{post.category}</span><h1>{post.title}</h1><p>{post.excerpt}</p><div className="blogArticleMeta"><span><UserRound size={15}/>{post.authorName}</span><span><CalendarDays size={15}/>{formatDate(post.publishedAt,locale)}</span><span><Clock3 size={15}/>{minutes} {c.min}</span></div>{post.coverImageUrl&&<img className="blogArticleCover" src={post.coverImageUrl} alt={post.coverImageAlt||post.title}/>}</header>
...
function readingMinutes(body:string){return Math.max(1,Math.ceil(body.trim().split(/\s+/).length/220));}
