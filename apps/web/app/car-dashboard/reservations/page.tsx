...
export default async function CarReservationsPage(){
  ...
  return <CarPartnerShell companyName={company.company.name} status={company.company.status} verified={company.company.verified} locale={market.baseLocale}>
    ...
    <section className={styles.reservationSummary} aria-label={ar?"ملخص الحجوزات":"Reservation summary"}>
      <div><span>{ar?"نشطة":"Active"}</span><strong>{activeCount}</strong></div>
      <div><span>{ar?"مكتملة":"Completed"}</span><strong>{completedCount}</strong></div>
      <div><span>{ar?"ملغية / لم يحضر":"Cancelled / no-show"}</span><strong>{cancelledCount}</strong></div>
      <div><span>{ar?"الإجمالي":"Total"}</span><strong>{reservations.length}</strong></div>
    </section>

    <section className={styles.panel}>{reservations.length?<div className={styles.tableWrap}><table className={`${styles.table} ${styles.reservationTable}`}><thead><tr><th>{ar?"المرجع":"Reference"}</th><th>{ar?"الضيف":"Guest"}</th><th>{ar?"السيارة":"Vehicle"}</th><th>{ar?"الاستلام":"Pickup"}</th><th>{ar?"التسليم":"Return"}</th><th>{ar?"المبلغ":"Amount"}</th><th>{ar?"الحالة":"Status"}</th><th>{ar?"إدارة":"Manage"}</th></tr></thead><tbody>{reservations.map((r)=><tr key={r.id}><td><strong>{r.reference}</strong></td><td>{r.guestName}<div>{r.guestEmail}</div>{r.guestPhone&&<div>{r.guestPhone}</div>}</td><td>{r.vehicle}</td><td>{r.pickupLocation}<div>{new Date(r.pickupAt).toLocaleString(ar?"ar-JO":"en-GB")}</div></td><td>{r.returnLocation}<div>{new Date(r.returnAt).toLocaleString(ar?"ar-JO":"en-GB")}</div></td><td><strong>{r.total.toFixed(2)} {r.currency}</strong><div>{ar?"وديعة":"Deposit"}: {r.deposit.toFixed(2)} {r.currency}</div></td><td><span className={styles.chip}>{statusLabel(r.status,ar)}</span></td><td><CarReservationActions reservationId={r.id} status={r.status} locale={market.baseLocale}/></td></tr>)}</tbody></table></div>:<div className={styles.empty}><span className={styles.emptyIcon}><CalendarRange size={25}/></span><h3>{ar?"لا توجد حجوزات سيارات بعد":"No car reservations yet"}</h3><p>{ar?"عند تأكيد أول حجز سيارة من الموقع سيظهر هنا تلقائيًا، بدون بيانات Demo أو إدخال يدوي.":"The first confirmed car booking from the public site will appear here automatically, with no demo data or manual entry."}</p></div>}</section>
  </CarPartnerShell>;
}
