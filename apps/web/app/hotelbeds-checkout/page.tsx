...
import {checkHotelbedsRate, createHotelbedsCheckoutToken, extractHotelbedsRateComments, getHotelbedsRateCommentDetails, paymentCapabilities, readHotelbedsCheckoutToken, type HotelbedsCheckoutHotel, type HotelbedsHotelDetails, type HotelbedsOffer} from "@platform/server";
...
export default async function HotelbedsCheckoutPage({searchParams}: {searchParams: Promise<SearchParams>}) {
    ...
    try {
      ...
      if (initialRateType === "RECHECK") {
        ...
        if (!checked || checked.offer.rateType.trim().toUpperCase() !== "BOOKABLE") {
          ...
          offer = checked.offer;
          hotel = hotelFromSnapshot({...snapshot.hotel, ...checked.hotel}, checked.offer);
        ...
        if (!rateComments && offer.rateCommentsId) rateComments = await getHotelbedsRateCommentDetails(offer.rateCommentsId, snapshot.stay.arrival, "ENG");
  ...
  return <main className="checkoutExperience" lang={market.intlLocale} dir={market.direction}>
    ...
    <section className="shell checkoutSection">
      {!snapshot || !hotel || !offer || !checkoutQuote ? <div className="premiumEmpty"><h3>{ar ? "سعر Hotelbeds لم يعد متاحًا" : "This Hotelbeds rate is no longer available"}</h3><p>{ar ? "ارجع إلى البحث واختر سعرًا مباشرًا جديدًا. إذا كان الحد اليومي للاختبار مستهلكًا، انتظر تجدد quota قبل إعادة المحاولة." : "Return to search and select a fresh provider rate. If the evaluation quota is exhausted, wait for it to reset before retrying."}</p><Link href="/search" className="resultCta">{ar ? "العودة إلى البحث" : "Return to search"}</Link></div> : <HotelbedsCheckoutFlow hotel={hotel} offer={offer} arrival={snapshot.stay.arrival} departure={snapshot.stay.departure} adults={snapshot.stay.adults} children={snapshot.stay.children} childrenAges={snapshot.stay.childrenAges} locale={market.locale} currency={market.currency} onlinePaymentAvailable={onlinePaymentAvailable} checkoutQuote={checkoutQuote} rateComments={rateComments}/>} 
    </section>
  </main>;
}
