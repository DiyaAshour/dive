...
export function HotelbedsCheckoutFlow({hotel, offer, arrival, departure, adults, children, locale, currency}: Props) {
  ...
  const canSubmit = useMemo(() => guestName.trim().length >= 2 && guestEmail.includes("@") && !busy && paymentMode === "PAY_AT_HOTEL", [guestName, guestEmail, busy, paymentMode]);
