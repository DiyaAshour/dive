...
import { z } from "zod";
import { buildImaginCatalogVehicle, getImaginCarListing, imaginConfigured, importCarCatalogVehicles, verifyExactImaginVehicle } from "@platform/server";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const verification = await verifyExactImaginVehicle(parsed.data.selection);
    if (!verification.exact) {
      return Response.json({
        data: null,
        error: {
          code: "CAR_VISUAL_EXACT_MATCH_REQUIRED",
          message: "The provider returned a substitute or unresolved vehicle. HandMeKey did not publish it.",
