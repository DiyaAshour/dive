...
import { createBookingHoldWithVisibilityBoost, VISIBILITY_BOOST_COOKIE, withBookingConcurrencyRetry } from "@platform/server";
import { handleApiError, ok, validationError } from "@/lib/api";
import { createDemoBookingHold } from "@/lib/demo-booking";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const demo = createDemoBookingHold(body.data);
    if (demo) return ok(demo, {status: 201});
    ...
    const result = await withBookingConcurrencyRetry(() => createBookingHoldWithVisibilityBoost(body.data, {
      userId: user?.id ?? null,
