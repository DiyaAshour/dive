...
export async function GET(request:NextRequest){
  try{
    ...
    for(const issue of queue.filter((item)=>item.title.startsWith(UPDATE_PREFIX))){
      ...
      if(!payload){
        rejections.push({issue:issue.number,reason:"Invalid SEO update queue JSON"});
        continue;
      }

      const generatedAt=Date.parse(payload.generatedAt);
      if(!Number.isFinite(generatedAt)){
        rejections.push({issue:issue.number,reason:"Invalid generatedAt timestamp"});
        continue;
      }

      const target=posts.find((post)=>post.locale===payload.locale&&post.slug===payload.targetSlug&&post.status==="PUBLISHED");
      if(!target){
        rejections.push({issue:issue.number,reason:"Target published article was not found"});
        continue;
      }
      ...
      if(slug!==payload.targetSlug){
        rejections.push({issue:issue.number,reason:"SEO refreshes cannot change the public slug"});
        continue;
      }
      ...
      return Response.json({
        ...
        generator:"ChatGPT GSC optimizer",
        openAiApiUsed:false,
...
function parseRawJson(body:string|null):unknown|null{
...
function parsePublishPayload(body:string|null):PublishPayload|null{
  const raw=parseRawJson(body);
  const parsed=publishPayloadSchema.safeParse(raw);
  return parsed.success?parsed.data:null;
}

function parseUpdatePayload(body:string|null):UpdatePayload|null{
  const raw=parseRawJson(body);
...
function qualityIssues(payload:QualityPayload,existing:ExistingPost[]){
  ...
  if(similarity>=0.68)issues.push(`Title is too similar to another article (similarity ${similarity.toFixed(2)})`);
