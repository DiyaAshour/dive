...
export function CarGallery({photos,spinFrames=[],locale,verified,vehicleName}:Props){
  ...
  const visiblePhotos=useMemo(()=>photos.filter((photo)=>photo.category!=="SPIN_FRAME"),[photos]);
  const effectiveSpinFrames=useMemo(()=>spinFrames.length?spinFrames:photos.filter((photo)=>photo.category==="SPIN_FRAME"),[photos,spinFrames]);
  const categories=useMemo(()=>Array.from(new Set(visiblePhotos.map((photo)=>photo.category))),[visiblePhotos]);
  const [filter,setFilter]=useState("ALL");
  const [mode,setMode]=useState<"PHOTO"|"SPIN">("PHOTO");
  const filtered=useMemo(()=>filter==="ALL"?visiblePhotos:visiblePhotos.filter((photo)=>photo.category===filter),[visiblePhotos,filter]);
  const [activeId,setActiveId]=useState(visiblePhotos[0]?.id??"");
  ...
  const spinFrame=effectiveSpinFrames[spinIndex]??effectiveSpinFrames[0]??null;
  const canSpin=effectiveSpinFrames.length>=8;

  function chooseFilter(value:string){setMode("PHOTO");setFilter(value);const next=value==="ALL"?visiblePhotos[0]:visiblePhotos.find((photo)=>photo.category===value);setActiveId(next?.id??"");}
  ...
  function moveSpin(event:React.PointerEvent){const start=dragStart.current;if(!start||!canSpin)return;const delta=event.clientX-start.x;const offset=Math.round(delta/8);const length=effectiveSpinFrames.length;setSpinIndex(((start.frame-offset)%length+length)%length);}
  ...
  const activeCatalog=active?isCatalogPhoto(active):false;
  ...
  return <div className={styles.gallery}>
    <div className={`${styles.stage} ${mode==="SPIN"?styles.spinStage:""}`} onTouchStart={(event)=>{if(mode==="PHOTO")touchStart.current=event.touches[0]?.clientX??null;}} onTouchEnd={onTouchEnd} onPointerDown={mode==="SPIN"?startSpin:undefined} onPointerMove={mode==="SPIN"?moveSpin:undefined} onPointerUp={mode==="SPIN"?stopSpin:undefined} onPointerCancel={mode==="SPIN"?stopSpin:undefined}>
      {mode==="SPIN"&&spinFrame?<img className={styles.catalogImage} src={spinFrame.url} alt={spinFrame.alt||vehicleName} draggable={false}/>:active?<img className={activeCatalog?styles.catalogImage:""} src={active.url} alt={active.alt||vehicleName}/>:<div className={styles.empty}><Camera size={44}/><strong>{copy.pending}</strong><span>{copy.pendingBody}</span></div>}
      <div className={styles.topbar}>{verified&&<span className={styles.badge}><ShieldCheck size={13}/>{copy.verified}</span>}{mode==="SPIN"&&spinFrame?<span className={styles.counter}>{spinIndex+1} / {effectiveSpinFrames.length}</span>:active?<span className={styles.counter}>{activeIndex+1} / {filtered.length}</span>:null}</div>
      {mode==="PHOTO"&&filtered.length>1&&<><button className={`${styles.navButton} ${styles.prev}`} type="button" aria-label="Previous photo" onClick={()=>step(-1)}><ChevronLeft size={20}/></button><button className={`${styles.navButton} ${styles.next}`} type="button" aria-label="Next photo" onClick={()=>step(1)}><ChevronRight size={20}/></button></>}
      ...
      {mode==="PHOTO"&&active?<span className={styles.sourceBadge}>{activeCatalog?copy.catalog:copy.supplier}</span>:null}
    </div>
    {(visiblePhotos.length>0||canSpin)&&<>
      <div className={styles.filters}>
        ...
        {visiblePhotos.length?<button className={mode==="PHOTO"&&filter==="ALL"?styles.active:""} type="button" onClick={()=>chooseFilter("ALL")}>{copy.all} · {visiblePhotos.length}</button>:null}
      ...
      {mode==="PHOTO"&&visiblePhotos.length>0?<div className={styles.thumbs}>{filtered.map((photo)=><button className={`${styles.thumb} ${photo.id===active?.id?styles.active:""}`} key={photo.id} type="button" onClick={()=>setActiveId(photo.id)}><img className={isCatalogPhoto(photo)?styles.catalogImage:""} src={photo.url} alt=""/><span>{labels[photo.category]??photo.category}</span></button>)}</div>:null}
    </>}
    {fullscreen&&active&&<div className={styles.overlay} role="dialog" aria-modal="true" aria-label={copy.open}>
      ...
      <div className={styles.overlayStage}><img className={activeCatalog?styles.catalogImage:""} src={active.url} alt={active.alt||vehicleName}/>{filtered.length>1&&<><button className={`${styles.navButton} ${styles.prev}`} type="button" onClick={()=>step(-1)}><ChevronLeft size={22}/></button><button className={`${styles.navButton} ${styles.next}`} type="button" onClick={()=>step(1)}><ChevronRight size={22}/></button></>}</div>
      <div className={styles.overlayThumbs}>{filtered.map((photo)=><button className={photo.id===active.id?styles.active:""} key={photo.id} type="button" onClick={()=>setActiveId(photo.id)}><img className={isCatalogPhoto(photo)?styles.catalogImage:""} src={photo.url} alt=""/></button>)}</div>
    </div>}
  </div>;
}

function isCatalogPhoto(photo:Photo){return photo.source==="CATALOG"||photo.id.startsWith("catalog-");}
