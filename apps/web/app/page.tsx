...
const jordanCityNamesAr: Readonly<Record<string, string>> = {
  Amman: "عمّان",
  Aqaba: "العقبة",
  Petra: "البتراء",
...
export default async function HomePage() {
  ...
  const destinationCopy = locale === "ar"
    ? {eyebrow: "اكتشف الأردن", title: "الوجهات الرائجة", intro: "استكشف المدن والوجهات الأكثر حضورًا في الأردن. ومع توسع HandMeKey سيعمل القسم نفسه لوجهات حول العالم.", stays: "إقامة متاحة", explore: "استكشف الإقامات"}
    : {eyebrow: "Explore Jordan", title: "Popular destinations", intro: "Explore the cities and destinations travelers are opening across Jordan. As HandMeKey expands, this same section can surface places worldwide.", stays: "stays available", explore: "Explore stays"};
  ...
  return <main className="customerPage">
    ...
    {destinations.length > 0 && <section className={`shell ${destinationStyles.destinationSection}`}>
      ...
      <div className={destinationStyles.destinationGrid}>{destinations.map((destination,index) => {
        const cityName = locale === "ar" ? jordanCityNamesAr[destination.city] ?? destination.city : destination.city;
        const countryName = regionNames.of(destination.countryCode) ?? destination.countryCode;
        return <Link prefetch={false} key={`${destination.countryCode}-${destination.city}`} className={`${destinationStyles.destinationCard} ${index < 2 ? destinationStyles.destinationCardFeatured : ""}`} href={`/search?destination=${encodeURIComponent(destination.city)}&arrival=${stay.arrival}&departure=${stay.departure}&adults=2&children=0`}>
