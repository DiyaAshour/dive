...
export default function SetupManager({hotelId,roomTypes,overbookingEnabled,locale}:{hotelId:string;roomTypes:RoomType[];overbookingEnabled:boolean;locale:Locale}) {
  ...
  return <div className="setupGrid" aria-label={ar?"إعداد الغرف والأسعار":"Rooms and rates setup"}>
    <section className="panel setupPanel"><span className="eyebrow">{ar?"الخطوة 2":"Step 2"}</span><h2>{ar?"أنواع الغرف":"Room types"}</h2><form onSubmit={createRoomType} className="stackForm"><input name="name" placeholder={ar?"غرفة ديلوكس كينغ":"Deluxe King"} required/><input name="code" placeholder="DLX-K" required/><div className="formGrid"><input name="maxAdults" aria-label={ar?"أقصى بالغين":"Maximum adults"} type="number" min="1" defaultValue="2" required/><input name="maxChildren" aria-label={ar?"أقصى أطفال":"Maximum children"} type="number" min="0" defaultValue="0" required/></div><button className="primaryButton">{ar?"إضافة نوع غرفة":"Add room type"}</button></form><div className="setupList">{roomTypes.map((room)=><div key={room.id}><strong>{room.name}</strong><span>{room.code} · {room.ratePlans.length} {ar?"خطط":"plans"}</span></div>)}</div></section>
