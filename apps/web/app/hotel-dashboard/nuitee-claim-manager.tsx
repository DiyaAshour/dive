...
type Props=Readonly<{hotelId:string;hotelName?:string;initialClaim:Claim|null;locale:string}>;

export default function NuiteeClaimManager({hotelId,hotelName="",initialClaim,locale}:Props){
  ...
  useEffect(()=>{
    if(!claim&&!query&&hotelName)setQuery(hotelName);
  },[claim,hotelName,query]);
  ...
  async function unlinkHotel(){
    ...
    try{
      ...
      setClaim(null);setQuery(hotelName);setMessage(ar?"تم إلغاء الربط. نسخة المورد قد تصبح مؤهلة للظهور مرة أخرى.":"Link removed. The supplier listing may become eligible to appear again.");router.refresh();
    }catch(error){setMessage(error instanceof Error?error.message:String(error));}
    finally{setBusyId(null);}
  }
  ...
  return <section className="partnerInsight" style={{alignItems:"flex-start",gap:16}}>
    <Building2 size={22}/><div style={{width:"100%",minWidth:0}}>
      <strong>{ar?"ابحث عن فندقك واربطه":"Find and connect your property"}</strong>
