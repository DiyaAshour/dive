...
export default async function PropertySettingsPage({searchParams}: {searchParams: Promise<{hotelId?: string}>}) {
  ...
  return <main className="partnerAppShell" dir={direction(locale)}>
    <PartnerSidebar hotelId={workspace.id} hotelName={workspace.name} city={workspace.city} status={workspace.status} active="property" locale={locale}/>
    <section className="partnerMain">
      <PartnerLanguageBar locale={locale}/>
      <div className="partnerTopbar"><div><span className="partnerPageEyebrow">{ar ? "إعدادات المنشأة" : "Property settings"}</span><h1>{workspace.name}</h1><p>{workspace.city} · {workspace.countryCode}</p></div><div className="partnerTopbarActions"><span className={`propertyStatus ${workspace.status.toLowerCase()}`}>{statusLabel(workspace.status, ar)}</span><Link className="partnerSecondaryAction" href="/partner/onboarding"><Plus size={16}/>{copy.addPropertyShort}</Link></div></div>

      <section className="dashboardStartHere" aria-label={ar ? "خطوات إعداد المنشأة" : "Property setup steps"}>
        <div className="dashboardStartHereHead"><div><span className="eyebrow">{workspace.status === "ACTIVE" ? (ar ? "إدارة المحتوى" : "Manage content") : (ar ? "ابدأ من هنا" : "Start here")}</span><h2>{workspace.status === "ACTIVE" ? (ar ? "عدّل الفندق بدون إرباك التشغيل اليومي" : "Edit the property without cluttering daily operations") : (ar ? "جهّز منشأتك بأربع خطوات واضحة" : "Set up your property in four clear steps")}</h2><p>{workspace.status === "ACTIVE" ? (ar ? "هذه الصفحة للمحتوى والإعدادات. التشغيل اليومي والحجوزات والأسعار بقيت في صفحاتها المخصصة." : "This page is for content and setup. Daily operations, reservations and revenue work stay in their dedicated screens.") : (ar ? "امشِ بالترتيب من 1 إلى 4. كل خطوة مكتملة تتحول إلى ✓، وقائمة النشر في آخر الصفحة تعرض فقط الأشياء التي ما زالت ناقصة." : "Work from step 1 to 4. Completed steps turn into a checkmark, and the publishing section only shows what is still missing.")}</p></div><div className="dashboardOverallProgress"><strong>{readinessDone}/{readiness.checks.length}</strong><span>{ar ? "متطلبات مكتملة" : "requirements complete"}</span></div></div>
