...
export default async function CheckoutPage({searchParams}:{searchParams:Promise<{hotelId?:string;roomTypeId?:string;ratePlanId?:string;arrival?:string;departure?:string}>}) {
  ...
  return <main className="checkoutExperience">
    ...
    <section className="shell checkoutSection">
      {complete?<CheckoutFlow hotelId={query.hotelId!} roomTypeId={query.roomTypeId!} ratePlanId={query.ratePlanId!} arrival={query.arrival!} departure={query.departure!} initialGuestName={user?.displayName??""} initialGuestEmail={user?.email??""}/>:<div className="premiumEmpty"><h3>A live room selection is required</h3><p>Choose a hotel, room and rate plan before opening checkout. HandMeKey does not keep a hidden demo checkout fallback.</p><Link href="/search" className="resultCta">Return to search</Link></div>}
    </section>
  </main>;
}
