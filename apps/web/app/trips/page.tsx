  ...
  return <main className="softBg">
    ...
    <section className="shell section">
      ...
      {sections.map(([state,title]) => {
        ...
        return <section key={state} style={{marginTop:28}}><div className="sectionHeading"><div><span className="eyebrow">{state}</span><h2>{title}</h2></div></div><div className="grid3">{items.map((trip)=><Link className="card" href={`/booking/${trip.id}`} key={trip.id}>
          {trip.hotel.coverPhoto ? <img src={trip.hotel.coverPhoto.url} alt={trip.hotel.coverPhoto.alt ?? trip.hotel.name}/> : <div style={{height:180,display:"grid",placeItems:"center",background:"#eef1f4"}} className="muted">No photo</div>}
