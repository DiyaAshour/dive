...
export default async function SearchPage({searchParams}: {searchParams: Promise<SearchParams>}) {
  ...
  return <main className="searchExperience">
    ...
    <section className="shell searchLayout">
      ...
      <div className="searchResults"><div className="searchResultsHead"><div><span className="eyebrow">{copy.search.live}</span><h1>{raw.destination}</h1><p>{data?.count ?? 0} {resultKind} {copy.search.for} {raw.arrival} → {raw.departure}</p></div>{parsed.success&&<SaveSearchButton locale={locale} destination={parsed.data.destination} arrival={parsed.data.arrival} departure={parsed.data.departure} adults={parsed.data.adults} children={parsed.data.children} filters={{minPrice:parsed.data.minPrice,maxPrice:parsed.data.maxPrice,stars:parsed.data.stars,amenities:parsed.data.amenities,freeCancellation:parsed.data.freeCancellation,paymentMode:parsed.data.paymentMode,sort:parsed.data.sort}}/>}</div>
