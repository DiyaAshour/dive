...
export default async function SearchPage({searchParams}: {searchParams: Promise<SearchParams>}) {
  ...
  return <main className="soft"><header className="shell topbar"><Link href="/" className="brand">B</Link><nav><Link href="/trips">My trips</Link><Link href="/account/alerts">Alerts</Link><Link href="/hotel-dashboard">Hotel dashboard</Link><Link href="/admin">Admin</Link></nav></header><section className="shell section">
    ...
    <div className="results"><aside className="panel filters"><span className="eyebrow">Filters</span>
    ...
    </aside><div><div className="sectionHead"><div><span className="eyebrow">Live search results</span><h2>{raw.destination} · {data?.count ?? 0} properties</h2></div>{parsed.success&&<SaveSearchButton destination={parsed.data.destination} arrival={parsed.data.arrival} departure={parsed.data.departure} adults={parsed.data.adults} children={parsed.data.children} filters={{minPrice:parsed.data.minPrice??null,maxPrice:parsed.data.maxPrice??null,stars:parsed.data.stars,amenities:parsed.data.amenities,freeCancellation:parsed.data.freeCancellation,paymentMode:parsed.data.paymentMode??null,sort:parsed.data.sort}}/>}</div>
      ...
      {data?.results.map((hotel)=><article className="resultCard" key={hotel.id}>
        ...
        <div className="resultBody"><div><div className="meta">{hotel.starRating ? `${hotel.starRating}★ · ` : ""}{hotel.area ? `${hotel.area} · ` : ""}{hotel.city}</div><h2>{hotel.name}</h2><p className="muted">Verified hotel · live inventory for the full stay</p>{hotel.reviewSummary.overall !== null && <p><strong>{hotel.reviewSummary.overall.toFixed(1)}/10</strong> <span className="muted">· {hotel.reviewSummary.count} verified review{hotel.reviewSummary.count === 1 ? "" : "s"}</span></p>}<div className="tags">{hotel.from.promotion && <span className="tag"><strong>{hotel.from.promotion.discountPercent}% deal</strong></span>}{hotel.amenities.slice(0,4).map((amenity)=><span className="tag" key={amenity.code}>{amenity.name}</span>)}<span className="tag">{hotel.from.cancellationPolicy.name}</span>{hotel.from.paymentModes.map((mode)=><span className="tag" key={mode}>{mode === "PAY_AT_HOTEL" ? "Pay at hotel" : "Pay now"}</span>)}</div>{hotel.from.availableToSell <= 3 && <div className="danger">Only {hotel.from.availableToSell} room{hotel.from.availableToSell === 1 ? "" : "s"} left for these dates</div>}</div><div className="rightPrice">{hotel.from.promotion && <small className="status">{hotel.from.promotion.name}</small>}<span className="muted">Average nightly total · taxes & service included</span><strong>{hotel.from.averageNightlyTotal.toFixed(2)} {hotel.currency}</strong><small className="muted">Stay total {hotel.from.total.toFixed(2)} {hotel.currency}</small><Link prefetch={false} href={hotelHref(hotel.id, parsed.success ? parsed.data : null)} className="primary">View live rooms</Link></div></div>
      </article>)}
    </div></div>
  </section></main>;
}
