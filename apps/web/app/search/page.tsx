...
export default async function SearchPage({searchParams}: {searchParams: Promise<SearchParams>}) {
  ...
  const raw = {
    ...
    infants: first(params.infants) ?? "0",
    pets: first(params.pets) ?? "0",
...
function hotelHref(hotelId:string,hotelSlug:string,input:{arrival:string;departure:string;adults:number;children:number;infants:number;pets:number}|null,campaignId?:string){if(!input)return `/hotel/${hotelSlug}`;const stay=new URLSearchParams({arrival:input.arrival,departure:input.departure,adults:String(input.adults),children:String(input.children),infants:String(input.infants),pets:String(input.pets)});if(!campaignId)return `/hotel/${hotelSlug}?${stay.toString()}`;const token=visibilityBoostAttributionToken(hotelId,campaignId);const click=new URLSearchParams({token,hotel:hotelSlug,...Object.fromEntries(stay.entries())});return `/api/v1/visibility-boost/click?${click.toString()}`;}
