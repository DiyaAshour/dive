...
type Props=Readonly<{hotelId:string;offerId:string;rateId:string;mappedRoomId:string;roomName:string;boardCode:string;boardName:string;policyName:string;selectedTotal:number;selectedCurrency:string;freeCancellation:boolean;hotelName:string;city:string;arrival:string;departure:string;adults:number;children:number;childrenAges:number[];guestNationality:string;locale:GuestLocale;currency:GuestCurrency;refreshHref:string;initialFirstName:string;initialLastName:string;initialEmail:string;accountPrefilled:boolean}>;
...
export function NuiteeCheckoutFlow(props:Props){
  const {hotelId,offerId,rateId,mappedRoomId,roomName,boardCode,boardName,policyName,selectedTotal,selectedCurrency,freeCancellation,hotelName,city,arrival,departure,adults,children,childrenAges,guestNationality,locale,currency,refreshHref,initialFirstName,initialLastName,initialEmail,accountPrefilled}=props;
    ...
    try{
      ...
      if(!session){
        const response=await fetch("/api/v1/nuitee/start-payment",{
          ...
          body:JSON.stringify({hotelId,offerId,rateId:rateId||undefined,mappedRoomId:mappedRoomId||undefined,roomName,boardCode:boardCode||undefined,boardName:boardName||undefined,policyName:policyName||undefined,selectedTotal,selectedCurrency,freeCancellation,hotelName,city,arrival,departure,adults,children,childrenAges,guestNationality,firstName:firstName.trim(),lastName:lastName.trim(),email:email.trim(),phone:phone.trim()||undefined}),
        });
