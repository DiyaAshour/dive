...
export default async function NuiteeCheckoutPage({searchParams}:{searchParams:Promise<SearchParams>}){
  ...
  return <main className="checkoutExperience" lang={market.intlLocale} dir={market.direction}><CustomerHeader minimal/><section className="checkoutBanner"><div className="shell"><div><span className="eyebrow">Nuitee Connect</span><h1>{ar?"راجع السعر المؤكد":"Review the confirmed rate"}</h1><p>{ar?"HandMeKey يعمل Prebook قبل الحجز النهائي للتأكد من السعر والتوافر.":"HandMeKey prebooks the offer before final booking to confirm price and availability."}</p></div><div className="checkoutTrust"><span><LockKeyhole size={18}/>{ar?"مفتاح API على الخادم فقط":"API key stays server-side"}</span><span><ShieldCheck size={18}/>{ar?"Prebook قبل التأكيد":"Prebook before confirmation"}</span></div></div></section><section className="shell checkoutSection">{!prebook||!hotel?<div className="premiumEmpty"><h3>{ar?"السعر لم يعد متاحاً":"This Nuitee offer is no longer available"}</h3><p>{ar?"ارجع إلى البحث واختر سعراً جديداً.":"Return to search and choose a fresh supplier rate."}</p><Link className="resultCta" href="/search">{ar?"العودة إلى البحث":"Return to search"}</Link></div>:<NuiteeCheckoutFlow prebookId={prebook.prebookId} hotelName={hotel.name} roomName={prebook.roomName} boardName={prebook.boardName} arrival={arrival} departure={departure} price={prebook.price} sourceCurrency={prebook.currency} locale={market.locale} currency={market.currency} sandbox={prebook.sandbox}/>}</section></main>;
}
