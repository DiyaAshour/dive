...
export function PriceWatch({locale,hotelId,arrival,departure,adults,children,currentTotal,currency}:Props){
  ...
  return <div className="panel" style={{marginTop:20}}><span className="eyebrow">{ar?"مراقبة السعر":"Price watch"}</span><h3>{ar?"تابع سعر هذه الإقامة تلقائيًا":"Track this stay automatically"}</h3><p className="muted">{ar?`أرخص سعر مباشر حاليًا: ${currentTotal.toFixed(2)} ${currency}. اترك الهدف فارغًا ليصلك تنبيه عند تسجيل سعر أدنى جديد.`:`Current cheapest live stay: ${currentTotal.toFixed(2)} ${currency}. Leave the target blank to be notified when a new lowest price is found.`}</p><div style={{display:"flex",gap:10,alignItems:"end",flexWrap:"wrap"}}><label style={{minWidth:220}}>{ar?"إجمالي مستهدف اختياري":"Optional target total"}<input type="number" min="0.01" step="0.01" value={target} onChange={(event)=>setTarget(event.target.value)} placeholder={currentTotal.toFixed(2)}/></label><button type="button" className="secondaryButton" disabled={busy} onClick={watch}>{busy?(ar?"جارٍ بدء المراقبة…":"Starting watch…"):(ar?"راقب هذا السعر":"Watch this price")}</button></div>{message&&<p className={success?"status":"danger"}>{message}</p>}</div>;
}
