...
import {createHotelbedsCheckoutToken, type HotelbedsHotelDetails, type HotelbedsOffer} from "@platform/server";
...
export function HotelbedsHotelPage({hotel, stay, market}: Readonly<{hotel: HotelbedsHotelDetails; stay: Stay; market: Market}>) {
  ...
  return <main className="hotelExperience" lang={market.intlLocale} dir={market.direction}>
    ...
    <section className="shell hotelDetailSection">
      ...
      {hotel.photos.length ? <div className="premiumGallery">{hotel.photos.slice(0, 5).map((photo, index) => <img className={`galleryPhoto${index + 1}`} key={`${photo.url}-${photo.sortOrder}`} src={photo.url} alt={photo.alt || hotel.name} loading={index === 0 ? "eager" : "lazy"} decoding="async" />)}</div> : <div className="hotelMediaEmpty"><span>{ar ? "التوافر والأسعار مباشرة من Hotelbeds." : "Live Hotelbeds inventory and provider rates."}</span></div>}
      <div className="hotelTrustBar"><span><ShieldCheck size={17}/>{ar ? "توافر مباشر" : "Live availability"}</span><span><BadgeCheck size={17}/>{ar ? "شروط الإلغاء من المزود" : "Provider cancellation terms"}</span><span><BadgeCheck size={17}/>{ar ? "CheckRate فقط عند طلب المزود" : "CheckRate only when required"}</span></div>
      ...
      <div className="rateSectionHead"><div><span className="eyebrow">{ar ? "أسعار Hotelbeds المباشرة" : "Hotelbeds live rates"}</span><h2>{hotel.offers.length} {ar ? "سعر متاح" : `available rate${hotel.offers.length === 1 ? "" : "s"}`}</h2><p>{ar ? "السعر BOOKABLE ينتقل مباشرة للحجز، والسعر RECHECK يتم فحصه مرة واحدة فقط قبل التأكيد." : "BOOKABLE rates go directly to booking; RECHECK rates are checked once before confirmation."}</p></div>{cheapest&&<div><span>{ar ? "ابتداءً من" : "From"}</span><strong>{guestMoney(cheapest.total,hotel.currency,market.currency,market.locale).text}</strong>{hotel.currency !== market.currency&&<small>{guestMoney(cheapest.total,hotel.currency,market.currency,market.locale).sourceText}</small>}<small>{ar ? "إجمالي الإقامة" : "stay total"}</small></div>}</div>
