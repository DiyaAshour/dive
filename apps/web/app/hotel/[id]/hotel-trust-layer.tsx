import { Activity, BadgeCheck, BedDouble, Car, Coffee, Dumbbell, HeartPulse, MapPin, Plane, ShieldCheck, Sparkles, Star, Utensils, Waves, Wifi } from "lucide-react";
...
export function HotelTrustLayer({hotel,reviews,locale}:{hotel:HotelDetails;reviews:ReviewData;locale:Locale}) {
  ...
  return <section className="hotelTrustLayer" aria-label={copy.overview}>
    ...
    {benefitGroups.length>0&&<section className="shortStaySection">
      <div className="sectionHeading"><span className="sectionKicker">{copy.shortStay}</span><h2>{copy.shortStayBenefits}</h2><p>{copy.realAmenities}</p></div>
      <div className="shortStayGrid">{benefitGroups.map((group)=><article key={group.kind}><div className={`benefitIllustration ${group.kind}`}>{benefitIcon(group.kind)}</div><h3>{group.title}</h3><ul>{group.items.slice(0,4).map((item)=><li key={item}><BadgeCheck size={14}/>{item}</li>)}</ul></article>)}</div>
    </section>}

    <section className="facilityShowcase">
      <div className="sectionHeading"><span className="sectionKicker">{copy.facilities}</span><h2>{copy.popularFacilities}</h2><p>{copy.facilitySource}</p></div>
      <div className="facilityIconGrid">{hotel.amenities.slice(0,12).map((amenity)=><div key={amenity.code}><span>{facilityIcon(amenity.code)}</span><strong>{amenityLabel(amenity.code,amenity.name,locale)}</strong></div>)}</div>
    </section>
