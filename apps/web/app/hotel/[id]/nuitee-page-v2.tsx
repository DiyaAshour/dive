              ...
              <section className="publicRateOptions">
                ...
                {group.offers.map((offer)=>{
                  ...
                  return <div className="publicRateRow" key={offer.offerId}>
                    ...
                    <div className="publicRateAction"><span>{ar?"الإجمالي النهائي":"Final stay total"}</span><div className="ratePrice"><strong>{total.converted?`${ar?"حوالي":"approx."} ${total.text}`:total.text}</strong><small>{nights} {nights===1?(ar?"ليلة":"night"):(ar?"ليالي":"nights")}</small></div><small>{average.converted?`${ar?"حوالي":"approx."} ${average.text}`:average.text} {ar?"متوسط الليلة":"average / night"}</small>{total.converted&&<small className="fxSourceAmount">{total.sourceText} · {ar?"يتم الحجز بعملة المزود الأصلية":"booking is charged in the provider currency"}</small>}{offer.availableToSell<=3&&<span className="rateInventory">{ar?`متبقي ${offer.availableToSell} فقط`:`Only ${offer.availableToSell} left`}</span>}<Link prefetch={false} className="bookRateButton" href={checkoutHref(hotel.providerHotelCode,offer.offerId,stay)}>{ar?"اختر هذا الخيار":"Choose this option"}</Link></div>
                  </div>;
                })}
              </section>
            </article>;
