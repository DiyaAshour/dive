          ...
          {!groups.length?<div className="premiumEmpty"><h3>{ar?"لا توجد غرف متاحة":"No rooms available"}</h3><p>{ar?"غيّر التواريخ أو عدد الضيوف وحاول مرة أخرى.":"Change the dates or guest count and try again."}</p></div>:<div className="roomOfferList" id="room-offers">{groups.map((group)=>{
            ...
            return <article className="roomOfferCard" key={group.key}>
              <section className="publicRoomProduct">
                <div className="publicRoomMedia">{roomPhoto?<img src={roomPhoto.url} alt={group.roomName}/>:<><ImageIcon size={28}/><span>{ar?"لم يرسل المزود صورة لهذه الغرفة":"Room photo not provided by supplier"}</span></>}</div>
              ...
              <section className="publicRateOptions">
                ...
                {group.offers.map((offer)=>{
                  ...
                  return <div className="publicRateRow" key={offer.offerId}>
                    <div className="publicRateCell packageCell"><span>{ar?"الغرفة":"Room"}</span><h4>{plan}</h4><div className="rateBadges">{offer.freeCancellationNow&&<span className="rateBadge success">{ar?"إلغاء مجاني الآن":"Free cancellation now"}</span>}{offer.availableToSell<=3&&<span className="rateBadge warning">{ar?`متبقي ${offer.availableToSell}`:`${offer.availableToSell} left`}</span>}</div></div>
                    ...
                    <div className="publicRateAction"><span>{ar?"الإجمالي النهائي":"Final stay total"}</span><div className="ratePrice"><strong>{total.converted?`${ar?"حوالي":"approx."} ${total.text}`:total.text}</strong><small>{nights} {nights===1?(ar?"ليلة":"night"):(ar?"ليالي":"nights")}</small></div><small>{average.converted?`${ar?"حوالي":"approx."} ${average.text}`:average.text} {ar?"متوسط الليلة":"average / night"}</small>{total.converted&&<small className="fxSourceAmount">{total.sourceText} · {ar?"يتم الحجز بعملة المزود الأصلية":"booking is charged in the provider currency"}</small>}{offer.availableToSell<=3&&<span className="rateInventory">{ar?`متبقي ${offer.availableToSell} فقط`:`Only ${offer.availableToSell} left`}</span>}<Link className="bookRateButton" href={checkoutHref(hotel.providerHotelCode,offer.offerId,stay)}>{ar?"اختر هذا الخيار":"Choose this option"}</Link></div>
                  </div>;
                })}
              </section>
            </article>;
