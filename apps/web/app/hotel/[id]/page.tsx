...
import { StayGuestPicker } from "@/components/stay-guest-picker";
...
export default async function HotelPage({params, searchParams}: {params: Promise<{id:string}>; searchParams: Promise<SearchParams>}) {
  ...
  const parsed=publicStaySchema.safeParse({arrival:first(query.arrival)??defaults.arrival,departure:first(query.departure)??defaults.departure,adults:first(query.adults)??"2",children:first(query.children)??"0",infants:first(query.infants)??"0",pets:first(query.pets)??"0"});
  const stay=parsed.success?parsed.data:{arrival:defaults.arrival,departure:defaults.departure,adults:2,children:0,infants:0,pets:0};
  const [hotel,reviewData]=await Promise.all([getPublicHotelDetails(id,stay),getPublicHotelReviews(id,6)]);
  const eligibleOffers=hotel.offers.filter((offer)=>offer.maxInfants>=stay.infants);
  const cheapest=eligibleOffers[0]??null;
  const roomGroups=groupOffers(eligibleOffers);
  ...
  const petFriendly=hasPetFriendlyAmenity(hotel.amenities);
  const guestSummary=guestMixLabel(market.baseLocale,stay.adults,stay.children,stay.infants,stay.pets);
  return <main className="hotelExperience" lang={market.intlLocale} dir={market.direction}>
    ...
    <section className="shell hotelDetailSection">
      <Link className="backLink" href={`/search?destination=${encodeURIComponent(hotel.city)}&arrival=${stay.arrival}&departure=${stay.departure}&adults=${stay.adults}&children=${stay.children}&infants=${stay.infants}&pets=${stay.pets}`}><ChevronLeft size={16}/>{interpolate(copy.hotel.back,{city:hotel.city})}</Link>
      ...
      <div className="availabilityCard"><div><span className="eyebrow">{copy.hotel.yourStay}</span><h2>{copy.hotel.chooseDates}</h2><p className="muted">{guestSummary}</p></div><form className="availabilityForm" method="get"><label><span>{copy.hotel.checkIn}</span><input name="arrival" type="date" defaultValue={stay.arrival} required/></label><label><span>{copy.hotel.checkOut}</span><input name="departure" type="date" defaultValue={stay.departure} required/></label><div className="hotelGuestPickerCell"><StayGuestPicker locale={market.baseLocale} adults={stay.adults} children={stay.children} infants={stay.infants} pets={stay.pets}/></div><button type="submit">{copy.hotel.availability}</button></form></div>
      {!parsed.success&&<div className="inlineWarning">{copy.hotel.invalidStay}</div>}
      {stay.pets>0&&<div className={petFriendly?"guestPolicyNotice positiveText":"inlineWarning"}>{petFriendly?(market.baseLocale==="ar"?"الفندق يعلن عن قبول الحيوانات الأليفة. راجع الشروط النهائية عند الحجز.":"This property advertises pet-friendly facilities. Final conditions are confirmed at booking."):(market.baseLocale==="ar"?"طلب الحيوان الأليف محفوظ، لكن الفندق لا يعلن سياسة قبول واضحة؛ يجب تأكيدها قبل الحجز.":"Your pet request is preserved, but this property does not publish a clear pet policy; confirm it before booking.")}</div>}
      {stay.infants>0&&eligibleOffers.length===0&&hotel.offers.length>0&&<div className="inlineWarning">{market.baseLocale==="ar"?"لا توجد غرفة متاحة بسعة رضّع مناسبة للعدد الذي اخترته.":"No available room supports the number of infants you selected."}</div>}
      {cheapest&&stay.infants===0&&stay.pets===0&&<PriceWatch locale={market.locale} hotelId={hotel.id} arrival={stay.arrival} departure={stay.departure} adults={stay.adults} children={stay.children} currentTotal={cheapest.total} currency={hotel.currency}/>} 
      <div className="hotelBookingWorkspace">
        <div className="hotelBookingMain">
          <div className="rateSectionHead"><div><span className="eyebrow">{copy.hotel.available}</span><h2>{eligibleOffers.length} {eligibleOffers.length===1?copy.hotel.liveRate:copy.hotel.liveRates}</h2><p>{hotel.stay.nights} {hotel.stay.nights===1?copy.hotel.night:copy.hotel.nights} · {guestSummary} · {copy.hotel.totalsNote}</p></div>{cheapestMoney&&<div><span>{copy.hotel.from}</span><strong>{cheapestMoney.converted?`${fxCopy.approx} ${cheapestMoney.text}`:cheapestMoney.text}</strong>{cheapestMoney.converted&&<small>{cheapestMoney.sourceText}</small>}<small>{copy.hotel.stayTotal}</small></div>}</div>
          {eligibleOffers.length===0?<div className="premiumEmpty"><h3>{copy.hotel.noRooms}</h3><p>{copy.hotel.noRoomsBody}</p></div>:<div className="roomOfferList" id="room-offers">{roomGroups.map(({room,offers})=>{
        ...
        {cheapest&&cheapestMoney&&<aside className="hotelBookingRail">
          ...
          <div className="hotelRailBody"><div className="hotelRailFacts"><div><span>{copy.hotel.checkIn}</span><strong>{stay.arrival}</strong></div><div><span>{copy.hotel.checkOut}</span><strong>{stay.departure}</strong></div><div><span>{roomUi.guests}</span><strong>{guestSummary}</strong></div><div><span>{roomUi.rateChoices}</span><strong>{eligibleOffers.length}</strong></div></div><div><a className="hotelRailButton" href="#room-offers">{roomUi.seeRoomsRates}</a><Link className="hotelRailLink" href={`/rewards/${market.baseLocale}`}>{roomUi.learnRewards}</Link><div className="hotelRailTrust"><span><ShieldCheck size={13}/>{copy.hotel.finalPrice}</span><span><BadgeCheck size={13}/>{copy.hotel.liveInventory}</span><span><BadgeCheck size={13}/>{copy.hotel.verifiedReviews}</span></div></div></div>
        </aside>}
      </div>
...
function checkoutHref(hotelId:string,roomTypeId:string,ratePlanId:string,arrival:string,departure:string,adults:number,children:number,infants:number,pets:number){const query=new URLSearchParams({hotelId,roomTypeId,ratePlanId,arrival,departure,adults:String(adults),children:String(children),infants:String(infants),pets:String(pets)});return `/checkout?${query.toString()}`;}
...
function hasPetFriendlyAmenity(amenities:Array<{code:string}>){const codes=new Set(amenities.map((amenity)=>amenity.code.trim().toUpperCase()));return ["PETS_ALLOWED","PET_FRIENDLY","PETS","PET_ALLOWED"].some((code)=>codes.has(code));}
function guestMixLabel(locale:"ar"|"en",adults:number,children:number,infants:number,pets:number){const parts=locale==="ar"?[`${adults} بالغ`,children?`${children} طفل`:null,infants?`${infants} رضيع`:null,pets?`${pets} حيوان أليف`:null]:[`${adults} ${adults===1?"adult":"adults"}`,children?`${children} ${children===1?"child":"children"}`:null,infants?`${infants} ${infants===1?"infant":"infants"}`:null,pets?`${pets} ${pets===1?"pet":"pets"}`:null];return parts.filter(Boolean).join(" · ");}
