...
export default async function ApiBookingsPage() {
  ...
  return <AdminShell locale={locale} principal={principal} active="api-bookings" counts={counts}>
    ...
    <section className="adminSection adminPanel"><div className="adminSectionTitle"><div><span className="eyebrow">{ar ? "سجل المزود" : "Provider ledger"}</span><h2>{ar ? "كل حجوزات Hotelbeds" : "All Hotelbeds bookings"}</h2><p>{ar ? "حجوزات API تبقى منفصلة عن حجوزات فنادق الشركاء، مع إمكانية مراجعة المرجع والحالة." : "API reservations stay separate from partner-property bookings, with provider reference and lifecycle status."}</p></div></div><div className="adminPropertyTable" role="table" aria-label={ar ? "حجوزات API" : "API bookings"}><div className="adminPropertyRow adminPropertyHead" role="row"><span>{ar ? "المرجع" : "Reference"}</span><span>{ar ? "الفندق" : "Hotel"}</span><span>{ar ? "الضيف" : "Guest"}</span><span>{ar ? "الإقامة" : "Stay"}</span><span>{ar ? "المبلغ" : "Amount"}</span><span>{ar ? "الحالة" : "Status"}</span></div>{bookings.length ? bookings.map((booking) => <div className="adminPropertyRow" role="row" key={booking.id}><div><strong>{booking.reference}</strong><small>Hotelbeds {booking.providerBooking.reference ?? booking.clientReference}</small></div><div><strong>{booking.hotel.name}</strong><small>{booking.hotel.city}</small></div><div><strong>{booking.guest.name}</strong><small>{booking.guest.email}</small></div><span>{booking.arrival} → {booking.departure}</span><span><strong>{booking.amounts.total.toFixed(2)} {booking.currency}</strong><small>{ar ? "هامش" : "Margin"}: {booking.amounts.markup.toFixed(2)}</small></span><span className={booking.status === "CONFIRMED" ? "statusOk" : "statusReview"}>{booking.status}</span></div>) : <div className="adminPropertyRow"><span>{ar ? "لا توجد حجوزات API بعد." : "No API bookings yet."}</span></div>}</div></section>
  </AdminShell>;
}
