...
import {BadgeCheck, Building2, CalendarRange, CarFront, CircleDollarSign, MapPin, Search} from "lucide-react";
...
export default async function AdminCarCompaniesPage({searchParams}: {searchParams: Promise<{q?: string; status?: string}>}) {
  ...
  return <AdminShell locale={locale} principal={principal} active="cars" counts={counts}>
    ...
    <nav className={styles.tabs}><Link href="/admin/cars"><CarFront size={15}/>{ar ? "نظرة عامة" : "Overview"}</Link><Link className={styles.active} href="/admin/cars/companies"><Building2 size={15}/>{ar ? "الشركات" : "Companies"}</Link><Link href="/admin/cars/reservations"><CalendarRange size={15}/>{ar ? "كل الحجوزات" : "All reservations"}</Link><Link href="/admin/cars/finance"><CircleDollarSign size={15}/>{ar ? "المالية" : "Finance"}</Link></nav>
    ...
    <section className="adminPanel adminSection">
      ...
      {companies.length ? <div className={styles.companyList}>{companies.map((company) => {
        ...
        const financeHref = `/admin/cars/finance?companyId=${encodeURIComponent(company.id)}`;
        return <article className={styles.companyCard} key={company.id}>
          ...
          <div className={`${styles.heroActions} ${styles.companyActions}`}><a className="secondaryButton" href={reservationsHref}>{ar ? "الحجوزات" : "Bookings"}</a><a className="secondaryButton" href={financeHref}>{ar ? "المالية" : "Finance"}</a><a className="primaryButton" href={companyHref}>{ar ? "إدارة" : "Manage"}</a></div>
        </article>;
