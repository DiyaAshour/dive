...
export default async function AdminBlogPage({searchParams}:{searchParams:Promise<Query>}){
  ...
  return <AdminShell locale={locale} principal={principal} active="blog" counts={counts}>
    <header className="adminTopbar blogAdminTopbar"><div><span className="eyebrow">{ar?"مركز المحتوى العضوي وSEO":"Organic content & SEO center"}</span><h1>{ar?"استوديو HandMeKey للمحتوى":"HandMeKey Content Studio"}</h1><p>{ar?"أنشئ أدلة السفر، ارفع صورها، ونظّمها داخل هيكل تصنيفات قابل للسحب والإفلات.":"Create travel guides, upload their images, and organize them in a drag-and-drop category hierarchy."}</p></div><Link className="primaryButton" href="/admin/blog/new"><Plus size={16}/>{ar?"مقال جديد":"New article"}</Link></header>
